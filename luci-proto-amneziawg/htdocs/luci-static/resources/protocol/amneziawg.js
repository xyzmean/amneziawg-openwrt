...
return network.registerProtocol('amneziawg', {
	...
	renderFormOptions: function(s) {
		...
		try {
			s.tab('peers', _('Peers'), _('Further information about AmneziaWG interfaces and peers at <a href=\'https://docs.amnezia.org/documentation/amnezia-wg\'>amneziawg.com</a>.'));
		}
		catch(e) {}
		...
		o.createPeerConfig = function(section_id, endpoint, ips, eips, dns) {
			...
			return [
				...
				jc && jc.length ? 'Jc = ' + jc : '# Jc not defined',
				jmin && jmin.length ? 'Jmin = ' + jmin : '# Jmin not defined',
				jmax && jmax.length ? 'Jmax = ' + jmax : '# Jmax not defined',
				s1 && s1.length ? 'S1 = ' + s1 : '# S1 not defined',
				s2 && s2.length ? 'S2 = ' + s2 : '# S2 not defined',
				h1 && h1.length ? 'H1 = ' + h1 : '# H1 not defined',
				h2 && h2.length ? 'H2 = ' + h2 : '# H2 not defined',
				h3 && h3.length ? 'H3 = ' + h3 : '# H3 not defined',
				h4 && h4.length ? 'H4 = ' + h4 : '# H4 not defined',
				'',
				'[Peer]',
				'PublicKey = ' + pub,
